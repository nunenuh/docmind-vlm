...
**Status:** Technical Design — Updated for Knowledge Base + RAG + Persona features

---

## 1. System Overview

DocMind-VLM is a dual-mode full-stack document intelligence platform:

1. **Document Extraction** — Upload → CV preprocessing → VLM extraction → structured fields with confidence scores and audit trails
2. **Knowledge Base** — Create projects → upload multiple docs → RAG indexing (pgvector) → persona-aware chat across all documents

The backend orchestrates two pipelines via LangGraph: a document processing pipeline (CV + VLM) and a RAG chat pipeline (embed → retrieve → reason → cite). All state is stored in Supabase Postgres (with pgvector extension for embeddings), files in Supabase Storage.
...
│                                                                    │
│  Landing ─── Dashboard ─┬── Extraction Workspace (single doc)      │
│                         │     ├── DocumentViewer (multi-page)      │
│                         │     ├── ExtractionPanel + Audit          │
│                         │     ├── ChatPanel (per-doc VLM chat)     │
│                         │     └── ComparePanel                     │
│                         │                                          │
│                         └── Project Workspace (Knowledge Base)     │
│                               ├── Document List (multi-doc)        │
│                               ├── Persona Selector/Editor          │
│                               ├── RAG Chat (cross-doc, persona)    │
│                               └── Conversation History             │
└──────────────┬──────────────────────┬──────────────────────────────┘
               │ REST API + SSE       │ Supabase Auth
...
│  │  ├─ extractions         │──┼──┼─▶│  ├─ projects, personas      │  │
│  │  ├─ projects (NEW)      │  │  │  │  ├─ page_chunks (pgvector)  │  │
│  │  ├─ personas (NEW)      │  │  │  │  ├─ conversations, messages │  │
│  │  └─ chat                │  │  │  │  └─ RLS per user            │  │
│  └────────┬───────────────┘  │  │  └────────────────────────────┘  │
│           │                  │  │                                  │
│  ┌────────▼───────────────┐  │  │  ┌────────────────────────────┐  │
│  │  Pipeline Layer         │  │  │  │  Storage                    │  │
│  │  (LangGraph)            │──┼──┼─▶│  (uploaded documents)       │  │
│  │                         │  │  │  └────────────────────────────┘  │
│  │  ┌─── Processing ────┐ │  │  │                                  │
│  │  │ preprocess (CV)    │ │  │  └──────────────────────────────────┘
│  │  │ extract (VLM)      │ │  │
...
│  │  └────────────────────┘ │  │   │  ├─ DashScope (Qwen3-VL)     │
│  │                         │  │   │  │   ├─ VLM (extract/chat)    │
│  │  ┌─── RAG (NEW) ─────┐ │  │   │  │   └─ Embeddings (v3)      │
│  │  │ text_extract       │ │  │   │  ├─ OpenAI (GPT-4o)          │
│  │  │ chunk              │─┼──┼──▶│  ├─ Google (Gemini)          │
│  │  │ embed              │ │  │   │  └─ Ollama (local)           │
│  │  │ retrieve (pgvec)   │ │  │   └───────────────────────────────┘
│  │  └────────────────────┘ │  │
