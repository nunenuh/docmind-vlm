...
import { usePersonas, useDeletePersona, useDuplicatePersona, useCreatePersona } from "@/hooks/usePersonas";
...
function PersonasTab() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-xs text-gray-500">{personas.length} personas ({presets.length} presets, {custom.length} custom)</p>
        <button
          onClick={() => setShowCreate(true)}
          className="flex items-center gap-2 px-3 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium rounded-lg transition-colors"
        >
          ...
          <div className="grid sm:grid-cols-2 gap-2">
            {presets.map((p: PersonaResponse) => (
              <PersonaCard
                key={p.id}
                persona={p}
                onDuplicate={() => duplicatePersona.mutate(p.id)}
              />
            ))}
          </div>
        </div>
      ...
      <div>
        ...
        {custom.length === 0 ? (
          <div className="bg-[#12121a] border border-[#1e1e2e] rounded-xl px-4 py-6 text-center">
            ...
            <p className="text-xs text-gray-600 mt-1">Click "Create Persona" or duplicate a preset</p>
          </div>
        ) : (
          <div className="grid sm:grid-cols-2 gap-2">
            {custom.map((p: PersonaResponse) => (
              <PersonaCard
                ...
                persona={p}
                onDuplicate={() => duplicatePersona.mutate(p.id)}
                onDelete={() => { if (window.confirm(`Delete "${p.name}"?`)) deletePersona.mutate(p.id); }}
              />
            ))}
          </div>
        )}
...
function CreatePersonaModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createPersona.mutate(
      { name: name.trim(), description, system_prompt: systemPrompt, tone, rules, boundaries },
      { onSuccess: () => onClose() },
    );
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose} />
      <div className="relative bg-[#12121a] border border-[#1e1e2e] rounded-2xl w-full max-w-lg mx-4 shadow-2xl max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#1e1e2e]">
          <h2 className="text-lg font-semibold text-white">Create Persona</h2>
          <button onClick={onClose} className="p-1.5 text-gray-400 hover:text-white rounded-lg hover:bg-white/5">
            <X className="w-5 h-5" />
          </button>
        </div>
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
          <div>
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g., Tax Advisor"
              className="w-full bg-[#0a0a0f] border border-[#2a2a3a] rounded-lg px-3 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500" autoFocus required />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1.5">Description</label>
          ...
          <div>
            ...
            <textarea value={systemPrompt} onChange={(e) => setSystemPrompt(e.target.value)} placeholder="You are a..."
              rows={4} className="w-full bg-[#0a0a0f] border border-[#2a2a3a] rounded-lg px-3 py-2.5 text-sm text-white placeholder-gray-500 resize-none focus:outline-none focus:border-indigo-500" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-gray-400 mb-1.5">Tone</label>
              <input value={tone} onChange={(e) => setTone(e.target.value)} placeholder="friendly, formal, ..."
                className="w-full bg-[#0a0a0f] border border-[#2a2a3a] rounded-lg px-3 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-400 mb-1.5">Rules</label>
              <input value={rules} onChange={(e) => setRules(e.target.value)} placeholder="Always cite sources, ..."
                className="w-full bg-[#0a0a0f] border border-[#2a2a3a] rounded-lg px-3 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500" />
            </div>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1.5">Boundaries</label>
            <input value={boundaries} onChange={(e) => setBoundaries(e.target.value)} placeholder="Do not provide legal advice, ..."
              className="w-full bg-[#0a0a0f] border border-[#2a2a3a] rounded-lg px-3 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500" />
          </div>
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose} className="px-4 py-2.5 text-sm text-gray-400 hover:text-white rounded-lg hover:bg-white/5">Cancel</button>
...
function PersonaCard({ persona, onDuplicate, onDelete }: { persona: PersonaResponse; onDuplicate?: () => void; onDelete?: () => void }) {
